...
;(function() {
  ...
  try {
    var WRITING_SEL = '[data-testid="chat-input"], .tiptap.ProseMirror';
    
    // --- PURE DETECTION CORE (inlined verbatim from js/core.js) ---
    ...
    function processInput() {
      document.querySelectorAll(WRITING_SEL).forEach(function(input) {
        ...
        if (dir === 'rtl') {
          input.dir = 'rtl'; input.style.direction = 'rtl'; input.style.textAlign = 'right'; input.style.paddingRight = '25px';
        } else {
    ...
    function init() {
      ...
      document.addEventListener('input', function(e) {
        ...
        var dir = detectTextDir(text);
        if (dir === 'rtl') {
          t.dir = 'rtl'; t.style.direction = 'rtl'; t.style.textAlign = 'right'; t.style.paddingRight = '25px';
        } else {
          if (t.hasAttribute('dir')) t.removeAttribute('dir');
